...
1. 이터레이터 획득: someIterable 객체의 [Symbol.iterator]() 메소드를 호출해서 이터레이터 객체(iterator)를 얻습니다. (제너레이터 객체는 자기 자신이 이터레이터이므로 그냥 자기 자신을 반환합니다.)
2. 반복 루프 시작: 내부적으로 while 루프와 비슷한 구조가 시작됩니다.
3. .next() 호출: 엔진이 우리가 보이지 않는 곳에서 이터레이터의 .next() 메소드를 호출합니다.
4. done 프로퍼티 확인: .next()가 반환한 객체(예: { value: 'peach', done: false })의 done 프로퍼티를 확인합니다.
5. 분기 처리:
done이 false이면:
반환된 객체의 value 프로퍼티를 for...of의 변수(예: val)에 할당합니다.
{ ... } 안의 코드를 실행합니다.
다시 3번 과정(.next() 호출)으로 돌아가 반복합니다.
done이 true이면:
루프를 즉시 종료합니다.
중요: for...of는 done: true일 때의 value는 무시하고 사용하지 않습니다. 이것이 바로 제너레이터의 return 값이 for...of에서 출력되지 않는 이유입니다.

스프레드 문법도 동일하게 작동함. 
반환된 value를 배열에 계속 push 한단 개념. 